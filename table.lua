...
function table.new(...)
	...
	-- [OPTIMIZATION] LuaJIT table.new(narray, nhash) signature detection
	-- If the first argument is a number, assume we are using the JIT pre-allocation constructor
	-- effectively acting as a proxy to table.new(narr, nrec)
	if type(arg1) == 'number' and table.new_jit then
		return setmetatable(table.new_jit(...), table)
	end

	-- Standard lua-ext constructor (Union of tables)
	-- Fallback to empty table or 0,0 pre-allocation
	local t
	if table.new_jit then
		t = table.new_jit(0, 0)
	else
		t = {}
	end
